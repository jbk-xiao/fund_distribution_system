<template>
    <div>
        ...
        <div class="inf">
            ...
            <el-row>
                <VisitLogTable ref="visit_log_list"></VisitLogTable>
            </el-row>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        async getData() {
            ...
            let res = await this.$http.get("salesManager/channelVisitLogList/" + this.channel_data.cid)
            ...
            if(res.data.code == 200) {
                this.$refs.visit_log_list.visit_log_list = res.data.data;
            }
        ...
        addVisitLog() {
            this.$router.push({
                ...
                query: { // 后续增加channel唯一id和manager唯一id
                    cid: this.channel_data.cid,
                    channel: this.channel_data.name,
                    organization: this.channel_data.organization,
                    mid: window.sessionStorage.getItem('userId'),
                    manager: window.sessionStorage.getItem('name'),
                    lid: window.sessionStorage.getItem('lid')
                },
            })
        },
    },
